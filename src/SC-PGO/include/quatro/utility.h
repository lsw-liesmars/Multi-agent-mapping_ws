...
#include "aloam_velodyne/cloud_info.h"
...
using QuatroPointType = pcl::PointXYZI;
#endif
...
inline void setCorrespondenceMarker(const pcl::PointCloud<QuatroPointType>& src_matched, const pcl::PointCloud<QuatroPointType>& tgt_matched,
    ...
    for (int idx = 0; idx < src_matched.size(); ++idx){
        QuatroPointType sP = src_matched[idx];
        QuatroPointType sT = tgt_matched[idx];
