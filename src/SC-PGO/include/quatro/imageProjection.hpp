...
private:
    ...
    aloam_velodyne::cloud_info segMsg;
...
public:
    ...
    ImageProjection(string lidarType, string neighborSelectionMode, string groundSegmentationMode, int numSubclusteringCriteria = 30) 
    {
        nanPoint.x         = std::numeric_limits<float>::quiet_NaN();
        nanPoint.y         = std::numeric_limits<float>::quiet_NaN();
        nanPoint.z         = std::numeric_limits<float>::quiet_NaN();
        ...
        else if(lidarType == "Ouster-OS1-128"){
            N_SCAN = 128;             
            Horizon_SCAN = 1024;
            ang_res_x = 360.0 / float(Horizon_SCAN);
            ang_res_y = 33.2 / float(N_SCAN - 1); 
        ...
        if (!(groundSegMode == "LeGO-LOAM") && !(groundSegMode == "Patchwork") && !(groundSegMode == "CSF")) {
            throw invalid_argument("[ImageProjection]: Check your paramter. Ground Segmentation mode is wrong!");
        }
    ...
    void setPointCloud(const pcl::PointCloud<QuatroPointType>::Ptr srcPtr) {
    ...
    void copyPointCloud(const pcl::PointCloud<QuatroPointType>::Ptr srcPtr) {
     ...
     * @param pcPtr
     */
    void segmentCloud(const pcl::PointCloud<QuatroPointType>::Ptr pcPtr) {
        ...
        if (groundSegMode == "LeGO-LOAM") {
            std::cout << "Ground Removal" << std::endl;
            groundRemoval();
