...
#pragma omp parallel for default(none) shared(N, v, vtilde, map)
    ...
    bool isReliableMatch(int num_rot_inliers_, int num_maxclique_, int rot_inliers_threshold, int maxclique_threshold) {
        return (num_rot_inliers_ >= rot_inliers_threshold) && (num_maxclique_ >= maxclique_threshold);
    }
    ...
    void computeTransformation(Eigen::Matrix4d &output) {
        ...
        if (!isReliableMatch(num_rot_inliers_, num_maxclique_, 20, 20)) { // 你可以根据经验设定阈值
            std::cout << "Match is not reliable, skipping this sample for loop closure." << std::endl;
            // output = Eigen::Matrix4d::Zero();  // 设置为无效矩阵
            return;
        }
    ...
    void setInliers(
            const Eigen::Matrix<double, 3, Eigen::Dynamic> &raw, pcl::PointCloud<QuatroPointType> &inliers,
            const vector<int> &idx_inliers) {
        inliers.clear();
        inliers.reserve(idx_inliers.size());

        for (const int idx: idx_inliers) {
            inliers.push_back(QuatroPointType(raw(0, idx), raw(1, idx), raw(2, idx)));
        }
    }

    void getMaxCliques(pcl::PointCloud<QuatroPointType> &source_max_clique,
                       pcl::PointCloud<QuatroPointType> &target_max_clique){
    ...
    void getFinalInliers(
            pcl::PointCloud<QuatroPointType> &source_inliers,
            pcl::PointCloud<QuatroPointType> &target_inliers) {
...
protected:
    ...
    pcl::PointCloud<QuatroPointType>               pcl_pruned_src_;
    pcl::PointCloud<QuatroPointType>               pcl_pruned_dst_;
