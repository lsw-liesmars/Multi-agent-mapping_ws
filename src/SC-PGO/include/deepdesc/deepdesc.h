...
public:
    ...
    void compute(std::vector<float> &input, std::vector<float> &descriptor) {
        ...
        torch::Tensor testeri = torch::from_blob(input.data(), {1, (int)num_points_, (int)input_dim_}, torch::kFloat).to(device);
    ...
    void compute(typename pcl::PointCloud<PointT>::ConstPtr queryscan, std::vector<float> &descriptor) {
        ...
        torch::Tensor tester = torch::zeros({1, (int)num_points_, (int)input_dim_}, torch::kFloat);
        for(size_t i=0; i<queryscan->size(); i++) {
            tester[0][i][0] = queryscan->points[i].x;
            tester[0][i][1] = queryscan->points[i].y;
            tester[0][i][2] = queryscan->points[i].z;
        }
...
public:
    ...
    DeepDescManager(const std::string &model_path, int numPoints = 4096, int inputDim = 3, int outputDim = 256) : modulePath_(model_path), numPoints_(numPoints), inputDim_(inputDim), outputDim_(outputDim)
    {
    ...
    void makeAndSaveDeepDescAndKeys(pcl::PointCloud<PointType> _scan_down);
