...
void matchPointClouds(pcl::PointCloud<QuatroPointType>::Ptr srcRaw, pcl::PointCloud<QuatroPointType>::Ptr tgtRaw, 
    ...
    // pcl::PointCloud<QuatroPointType>::Ptr ptrSrcNonground(new pcl::PointCloud<QuatroPointType>);
    // pcl::PointCloud<QuatroPointType>::Ptr ptrTgtNonground(new pcl::PointCloud<QuatroPointType>);

    pcl::PointCloud<QuatroPointType> srcInvalidSegments, tgtInvalidSegments;
    pcl::PointCloud<QuatroPointType>::Ptr srcValidSegments(new pcl::PointCloud<QuatroPointType>);
    pcl::PointCloud<QuatroPointType>::Ptr tgtValidSegments(new pcl::PointCloud<QuatroPointType>);
    ...
    if (groundSegMode == "LeGO-LOAM") {
        ...
        processPatchwork(lidarType, neighborSelectionMode, srcRaw, srcGround, ptrSrcNonground, srcValidSegments, srcInvalidSegments);
        processPatchwork(lidarType, neighborSelectionMode, tgtRaw, tgtGround, ptrTgtNonground, tgtValidSegments, tgtInvalidSegments);
    } else if (groundSegMode == "CSF") {
        // std::cout << "Ground Segmentation Mode: CSF" << std::endl;
        {
            std::lock_guard<std::mutex> lock(csfMutex);
            // applyCSFFilter(srcRaw, srcGround, ptrSrcNonground, csf_params);
            // applyCSFFilter(tgtRaw, tgtGround, ptrTgtNonground, csf_params);
    ...
    std::chrono::system_clock::time_point after_csf = std::chrono::system_clock::now();

    // // without removing ground points
    ...
    *tgtMatched = fpfhmanager.getTgtKps();
    ...
    // if (output.isZero(1e-6)) {
    //     return;
    // }


    std::chrono::duration<double> sec = std::chrono::system_clock::now() - start;
    std::chrono::duration<double> csf_sec = after_csf - start;
    ...
    // std::cout << "CSF: " << csf_sec.count() << " sec. \n";
