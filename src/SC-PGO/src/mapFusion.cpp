...
inline gtsam::Pose3_ inversePose3(const gtsam::Pose3_& pose_expr) {
    return gtsam::Expression<gtsam::Pose3>(
        pose_expr, &gtsam::Pose3::inverse
    );
}
...
inline gtsam::Pose3_ transformTo(const gtsam::Pose3_& x, const gtsam::Pose3_& p) {
    ...
    // return gtsam::compose(x, p);
    return inversePose3(x) * p;
    // return x.between(p);
    // return gtsam::transformFrom(x, p);
    // return gtsam::compose(p, gtsam::inverse(x));
}
...
private:
    ...
    void initialization(){
        ...
        // _downsize_filter_icp.setLeafSize(0.2, 0.2, 0.2);
    ...
    int robotID2Number(std::string robo){
        if (isdigit(robo.back())) {
            return robo.back() - '0';
        } else {
            return toupper(robo.front()) - 'A';
        }
    }
...
//        return;
    ...
    void gtsamFactorGraph(){
        ...
        for(auto ite : _global_map_trans){
            ...
            gtsam::Pose3 measurement_latest = gtsam::Pose3 (gtsam::Rot3::RzRyRx(pclpose.roll, pclpose.pitch, pclpose.yaw),
                                                            gtsam::Point3(pclpose.x, pclpose.y, pclpose.z));
            if(id_0 == 0){
                if (!initialEstimate.exists(id_1)) {
                    initialEstimate.insert(id_1, measurement_latest);
                } else {
                    ROS_WARN_STREAM("Key already exists: " << id_1);
                }
                ill_posed = false;
            }
            else
        ...
        for(auto ite: _global_odom_trans){
            ...
            for(auto ite_measure: ite.second){
                ...
                gtsam::Pose3 measurement = gtsam::Pose3 (gtsam::Rot3::RzRyRx(pclpose.roll, pclpose.pitch, pclpose.yaw),
                                                         gtsam::Point3(pclpose.x, pclpose.y, pclpose.z));
                // Vector6 << 1, 1, 1, 1, 1, 1;
                Vector6 <<  10, 10, 10, 10, 10, 10; //TODO: tune this value
                auto odometryNoise = gtsam::noiseModel::Diagonal::Variances(Vector6);
                // graph.add( gtsam::BetweenFactor<gtsam::Pose3>(id_0, id_1, measurement, odometryNoise) );
            }
        ...
        while (!terminate_signal){
            ...
            for(auto id = id_received.begin(); id != id_received.end();)
            {
                ...
                gtsam::Pose3 pose_t;
                try {
                    pose_t = initialEstimate.at<gtsam::Pose3>(id_t);
                } catch (const std::exception& e) {
                    ROS_WARN_STREAM("Failed to access id_t in initialEstimate: " << e.what());
                    continue;
                }
                ...
                if ( id_this == get<1>(*it)){
                    ...
                    if (!initialEstimate.exists(id_this)) {
                        ...
                        ROS_WARN_STREAM("Key already exists: " << id_this);
                    }
    ...
    void run(ScanContextBin bin){
        ...
        if(!getInitialGuesses(bin)){
            if(_idx_nearest_list.size() > 0){
               std::cout << "No initial guess found!" << std::endl; 
            }
            return;
        }

        if(!incrementalPCM()){
            std::cout << "incrementalPCM failed!" << std::endl;
            return;
        }

        std::cout << "[Loop closure succeeded] Current robot: " << bin.robotname
              << " Time: " << bin.time
              << " Number of candidates: " << _idx_nearest_list.size()
              << std::endl;
    ...
    void KNNSearch(ScanContextBin bin){
        ...
        for (int i = 0; i < std::min(_num_match_candidates, int(idx_list.size())); i++)
        {
            ...
            // Print the robot ID of the current and loop closure candidate
            // ROS_INFO("Current robot ID: %s, Loop closure candidate robot ID: %s",
            //          bin.robotname.c_str(), _bin_with_id.at(idx_candidate).robotname.c_str());
        }
        // std::cout << "Nearest list size: " << _idx_nearest_list.size() << std::endl;
        // TODO yiwen: 打印闭环对应的机器人ID
        idx_list.clear();
    }
    ...
    bool getInitialGuess(ScanContextBin bin, int idx_nearest, int min_idx){
        ...
        if (_global_map_trans_optimized.find(_robot_this_th) != _global_map_trans_optimized.end()){
            ...
            std::cout << "sc_pitch: " << sc_pitch << std::endl;
        ...
        if (pose_source_lidar.intensity == -1 || pose_source_lidar.intensity > _icp_thres)
            // std::cout << "pose_source_lidar.intensity: " << pose_source_lidar.intensity << std::endl;
            return false;
...
#pragma omp parallel for num_threads(numberOfCores)
                                               ...
                                               PointTypePose pose_source)
    {
        ...
        icp.setTransformationEpsilon(1e-5);  //1-5
        icp.setEuclideanFitnessEpsilon(1e-5);  //1-5
...
//        }
    ...
    void gtsamExpressionGraph(){
        ...
        if (!initial.exists(0)) {
            ...
            ROS_WARN_STREAM("Key already exists: 0");
        }
        //initial.print();
        ...
        for (auto i:_loop_accept_queue[_robot_this_th]){
            ...
            //     noiseScore;
            Vector6 << 0.01, 0.01, 0.01, 0.01, 0.01, 0.01; // Set a small noise for the example
