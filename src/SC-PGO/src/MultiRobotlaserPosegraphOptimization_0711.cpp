...
public:
    ...
    std::optional<gtsam::Pose3> doICPVirtualRelativeFriendRobot(Robot* curr_robot, Robot* friend_robot, int curr_node_idx, int friend_node_idx)
    {
        ...
        // Eigen::Affine3f guess = toEigenAffine3f(loopKfPose).inverse() * toEigenAffine3f(curr_robot->keyframePosesUpdated[curr_node_idx]); // TODO yiwen: use SC guess
        // curr_robot->mKF->unlock();
        // friend_robot->mKF->unlock();
        icp.align(*unused_result, guess.matrix());
        ...
        // test
        // Eigen::Matrix3f rot;
        // rot = Eigen::AngleAxisf(M_PI / 4, Eigen::Vector3f::UnitZ()) * // 45度旋转绕Z轴
        // Eigen::AngleAxisf(M_PI / 4, Eigen::Vector3f::UnitY()) * // 45度旋转绕Y轴
        // Eigen::AngleAxisf(M_PI / 4, Eigen::Vector3f::UnitX());  // 45度旋转绕X轴
        // Eigen::Vector3f trans;
        // trans << 1.0, 2.0, 3.0; // 设置位移向量为 (1, 2, 3)
        ...
        while (1)
        {
            while (!odometryBuf.empty() && !fullResBuf.empty())
            {
                ...
                *hasNewScanForLC = true;
                ...
                if (!(*gtSAMgraphMade))
                {
                    if(robot_id==0){
                        ...
                        mtxPosegraph->lock();
                        {
                            ...
                            // poseTo = Pose6DtoGTSAMPose3(Pose6D{3, 3, 0, 0, 0, 0});  // TODO yiwen: use cloudcompare relative pose
                            // auto relative_pose = poseOrigin.between(poseTo);

                            // for rooftop data
                            Eigen::Matrix4d T;
                            if (robot_id == 1){
                                T << 0.732140839100, -0.681153297424, 0.000000000000, 12.195608139038,
                                 0.681153297424, 0.732140839100, 0.000000000000, -7.318109512329,
                ...
                pcl::io::savePCDFileBinary(pgScansDirectory + save_pcd_index + ".pcd", *thisKeyFrame); // TODO yiwen: t_sec.pcd
    ...
    std::vector<std::tuple<int, int, int, float>> detectLoopClosureIDFriendRobot(Robot& robot, std::unordered_map<int, std::shared_ptr<Robot>>& friend_robots) {
        ...
        for(auto& [friend_robot_id_tmp, friend_robot] : friend_robots) {
            ...
            // std::cout << "robot_" << robot.robot_id << " and " << "robot_" << friend_robot->robot_id << " min_dist: " << min_dist << std::endl;
        ...
        while (ros::ok())
        {
            ...
            if (*gtSAMgraphMade && newStateTimes.size() > 0)
            {
                ...
                // {
                    ...
                    // saveOptimizedVerticesTUMFormat(isamCurrentEstimate, pgTUMFormat, isamStateTimes, robot_id); // TODO yiwen: same time as t_sec
                //     break;
                // }
            }
        }
