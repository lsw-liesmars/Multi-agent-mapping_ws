...
struct PoseChangeStats {
    bool hasCountChanged;
    int countDifference;
    double maxPositionChange;
    double avgPositionChange;
    double maxRotationChange;
    double avgRotationChange;
};

/**
 * 比较当前位姿估计与上次保存的位姿估计的变化
 * @param currentEstimates 当前的位姿估计
 * @param lastSavedEstimates 上次保存的位姿估计
 * @param currentTimes 当前的时间戳数组
 * @param lastSavedTimes 上次保存的时间戳数组
 * @return 位姿变化统计信息
 */
PoseChangeStats comparePoseChanges(
    const gtsam::Values &currentEstimates, const gtsam::Values &lastSavedEstimates)
{
    using namespace gtsam;
    
    PoseChangeStats stats;
    stats.hasCountChanged = (currentEstimates.size() != lastSavedEstimates.size());
    stats.countDifference = static_cast<int>(currentEstimates.size()) - static_cast<int>(lastSavedEstimates.size());
    stats.maxPositionChange = 0.0;
    stats.avgPositionChange = 0.0;
    stats.maxRotationChange = 0.0;
    stats.avgRotationChange = 0.0;
    
    // 如果是首次比较或者没有共同的位姿，则返回
    if (lastSavedEstimates.empty()) {
        return stats;
    }
    
    double totalPositionChange = 0.0;
    double totalRotationChange = 0.0;
    int comparedCount = 0;
    
    // 比较共同的位姿
    size_t minSize = std::min(currentEstimates.size(), lastSavedEstimates.size());
    
    for (size_t i = 0; i < minSize; ++i) {
        Key key = i; // 假设键是从0开始的连续整数
        
        if (!currentEstimates.exists(key) || !lastSavedEstimates.exists(key))
            continue;
        
        auto pCurrent = dynamic_cast<const GenericValue<Pose3>*>(&currentEstimates.at(key));
        auto pLast = dynamic_cast<const GenericValue<Pose3>*>(&lastSavedEstimates.at(key));
        
        if (!pCurrent || !pLast)
            continue;
        
        const Pose3& poseCurrent = pCurrent->value();
        const Pose3& poseLast = pLast->value();
        
        // 计算位置变化
        Point3 transCurrent = poseCurrent.translation();
        Point3 transLast = poseLast.translation();
        double posChange = (transCurrent - transLast).norm();
        
        // 计算旋转变化（四元数角度距离）
        Eigen::Quaterniond qCurrent = poseCurrent.rotation().toQuaternion();
        Eigen::Quaterniond qLast = poseLast.rotation().toQuaternion();
        double rotChange = qCurrent.angularDistance(qLast);
        
        totalPositionChange += posChange;
        totalRotationChange += rotChange;
        
        stats.maxPositionChange = std::max(stats.maxPositionChange, posChange);
        stats.maxRotationChange = std::max(stats.maxRotationChange, rotChange);
    ...
    // 计算平均变化
    if (comparedCount > 0) {
        stats.avgPositionChange = totalPositionChange / comparedCount;
        stats.avgRotationChange = totalRotationChange / comparedCount;
    }
    
    return stats;
}

bool IfSignificantChanges(
    const gtsam::Values &currentEstimates,
    const gtsam::Values &lastSavedEstimates,
    double posThreshold = 0.05,
    double rotThreshold = 0.02)
{
    // If this is the first call or no data to compare, save and return true
    if (lastSavedEstimates.empty()) {
        return true;
    }
    ...
    // Compare changes
    PoseChangeStats stats = comparePoseChanges(currentEstimates, lastSavedEstimates);
    
    // Determine if changes are significant
    bool significantChanges = stats.hasCountChanged || 
                              stats.maxPositionChange > posThreshold ||
                              stats.maxRotationChange > rotThreshold;
    
    if (significantChanges) {
        // ROS_INFO("Significant pose changes detected: new poses=%d, max position change=%.4f m, max rotation change=%.4f rad",
        //         stats.countDifference, stats.maxPositionChange, stats.maxRotationChange);
        return true;
    }
...
public:
    ...
    gtsam::Values last_isamCurrentEstimate_;
        ...
        kdtreeHistoryKeyPoses(new pcl::KdTreeFLANN<pcl::PointXYZ>())
    {
        ...
        last_isamCurrentEstimate_ = gtsam::Values();
    ...
    void initializeQuatroParams(ros::NodeHandle &nh) 
    {
        ...
        std::cout << "useQuatro: " << useQuatro << ", filterGround: " << filterGround << std::endl;
        if (!useQuatro) {
            return;
        }
        try {
    ...
    void init_params(ros::NodeHandle &nh, int id)
    {
        ...
        save_directory += "/robot_" + std::to_string(id+1) + "/";
        auto unused = system((std::string("mkdir -p ") + save_directory).c_str());

        pgTUMFormat = save_directory + "robot" + std::to_string(id+1)+".txt"; // optimized poses
        ...
        pgScansDirectory = save_directory + "robot" + std::to_string(id+1) + "_Scans/";
   ...
   void loopFindNearKeyframesCloud(pcl::PointCloud<PointType>::Ptr &nearKeyframes, pcl::PointCloud<PointType>::Ptr &nearKeyframes_DS,
    ...
    void logICPTransform(int cur_global_idx, int friend_global_idx, const Eigen::Matrix4f& icp_trans, const std::string& save_path)
    {
        ...
        std::ofstream outfile;
        outfile.open(save_path, std::ios_base::app);
        outfile << cur_global_idx << " " << friend_global_idx << " "
                << t.x() << " " << t.y() << " " << t.z() << " "
    ...
    std::optional<gtsam::Pose3> doICPVirtualRelativeFriendRobot(Robot* curr_robot, Robot* friend_robot, int curr_node_idx, int friend_node_idx)
    {
        ...
        std::cout << " strat inter icp" << std::endl;
        ...
        if (!icp.hasConverged() || icp.getFitnessScore() > curr_robot->Multi_loopFitnessScoreThreshold) {
            if (choose_quatro){
                std::string save_path = save_directory +"/failed_quatro.txt";
                logRotationAngleAndRobots(cur_global_idx, friend_global_idx, initial_guess, icp.getFinalTransformation(), icp.getFitnessScore(), save_path);
            }
            return std::nullopt;
        } else {
            ...
            std::string save_path = save_directory + "/successed_res.txt";
            // if(choose_quatro){
                logRotationAngleAndRobots(cur_global_idx, friend_global_idx, initial_guess, icp.getFinalTransformation(), icp.getFitnessScore(), save_path);
            // }
            std::string interTrans_path = save_directory + "/successed_trans.txt";
            logICPTransform(cur_global_idx, friend_global_idx, icp.getFinalTransformation(), interTrans_path);
        }
    ...
    std::tuple<int, int, int, float> detectDeepLoopFriendRobot(Robot& robot, std::shared_ptr<Robot>& friend_robot) {
        ...
        auto curr_key = robot.deepDescManager.deepdesc_invkeys_mat_[robot.last_inter_lcd_size];
    ...
    void visualizeLoopClosure()
    {
        ...
        double line_width;
        ros::param::param("~loop_line_width", line_width, 0.5); // 默认值为 0.5
        markerEdge.scale.x = line_width;
    ...
    void visualizeInterRobotLoopClosure()
    {
        ...
        double line_width;
        ros::param::param("~inter_loop_line_width", line_width, 0.5); // 默认值为 0.5
        interMarkerEdge.scale.x = line_width;
        ...
        while (ros::ok())
        {
            while (!initLoopICPBuf.empty())
            {
                ...
                if (relative_pose_optional)
                {
                    ...
                    std::ofstream outfile;
                    outfile.open(loop_path, std::ios_base::app);

                    auto prev_node_idx_in_dense = keyframeIndexTOallID[prev_node_idx];
                    auto curr_node_idx_in_dense = keyframeIndexTOallID[curr_node_idx];
                    outfile << "prev_node_idx_in_dense:" << prev_node_idx_in_dense << ",  curr_node_idx_in_dense:" << curr_node_idx_in_dense << "\n";
                    outfile.close();
    ...
    void process_inter_icp()
    {
        ...
        while (ros::ok())
        {
            ...
            for (const auto &loop_idx_tuple : tasks)
            {
                ...
                std::string inter_loop_path = save_directory + std::to_string(robot_id) + "_interloop.txt";
                std::ofstream outfile;
                outfile.open(inter_loop_path, std::ios_base::app);
                ...
                std::cout << "finished doICPVirtualRelativeFriendRobot" << std::endl;
                if (inter_relative_pose_optional)
                {
                    gtsam::Pose3 inter_relative_pose = inter_relative_pose_optional.value();

                    // 存储样本对
                    std::string icp_sample_path = save_directory + "icp_success_pairs.txt";
                    std::ofstream icp_sample_file(icp_sample_path, std::ios_base::app);
                    icp_sample_file << curr_robot_id << " " << curr_node_idx << " "
                                    << friend_robot_id << " " << friend_node_idx << std::endl;
                    icp_sample_file.close();
    ...
    void process_isam()
    {
        ...
        while (ros::ok())
        {
            ...
            if (*gtSAMgraphMade)  //  && newStateTimes.size() > 0
            {
                ...
                // // {
                // std::cout<<"if save!\n";
                if (IfSignificantChanges(isamCurrentEstimate, last_isamCurrentEstimate_)){
                    ...
                    last_isamCurrentEstimate_ = isamCurrentEstimate;
                    last_save_time_ = ros::Time::now();
                    is_finished_ = false;
                }
                else if(ros::Time::now() - last_save_time_ > ros::Duration(20.0)){
                    is_finished_ = true;
                }
                    // break;
                // }
                
                is_finished_ = is_finished_&& initLoopICPBuf.empty() && interRobotLoopICPBuf.empty();
                is_finished_ = is_finished_ && (last_inter_lcd_size >= deepDescManager.deepdesc_invkeys_mat_.size());

                bool all_finished = is_finished_;
                for (const auto& [id, robot_ptr] : friend_robots) {
                    if (!robot_ptr->is_finished_) {
                        all_finished = false;
                        break;
                    }
                }
                // if (all_finished) {
                //     // kill roscore and rosmaster, and cur node
                //     std::cout << "All robots finished, killing roscore and rosmaster..." << std::endl;
                //     std::string command = "killall -9 roscore rosmaster";
                //     system(command.c_str());
                //     command = "killall -9 rviz";
                //     system(command.c_str());
                //     command = "killall -9 rosbag";
                //     system(command.c_str());
                //     command = "killall -9 rosnode";
                //     system(command.c_str());
                //     command = "killall -9 roslaunch";
                //     system(command.c_str());
                ...
                // {
                //     // 保存最终结果
                //     std::cout << "All robots finished, saving final results and killing processes..." << std::endl;
                ...
                //     // 关闭ROS相关进程
                //     std::string command = "killall -9 roscore rosmaster rviz rosbag rosnode roslaunch";
                //     system(command.c_str());

                //     // 强制终止当前程序，使用exit()
                //     std::cout << "Exiting program..." << std::endl;
                //     exit(0); // 使用exit(0)强制终止程序
                // }
            }
        }
    }
...
int main(int argc, char **argv)
{
    ...
    int num_robots; // 假设有三个机器人
    nh.param<int>("num_robots", num_robots, 3);
    ...
    for (int i = 0; i < num_robots; ++i)
    {
        std::string odom_topic, full_cloud_topic, gps_topic;
        nh.param<std::string>("robot" + std::to_string(i) + "_odom_topic", odom_topic, "/robot" + std::to_string(i) + "/odom");
        nh.param<std::string>("robot" + std::to_string(i) + "_full_cloud_topic", full_cloud_topic, "/robot" + std::to_string(i) + "/full_cloud");
        nh.param<std::string>("robot" + std::to_string(i) + "_gps_topic", gps_topic, "/robot" + std::to_string(i) + "/gps");
    
        odometry_subscribers.push_back(nh.subscribe<nav_msgs::Odometry>(
            odom_topic, 100, [i, &robots](const nav_msgs::Odometry::ConstPtr& msg) { robots[i]->laserOdometryHandler(msg); }
        ));
        fullRes_subscribers.push_back(nh.subscribe<sensor_msgs::PointCloud2>(
            full_cloud_topic, 100, [i, &robots](const sensor_msgs::PointCloud2ConstPtr& msg) { robots[i]->laserCloudFullResHandler(msg); }
        ));
        gps_subscribers.push_back(nh.subscribe<sensor_msgs::NavSatFix>(
            gps_topic, 100, [i, &robots](const sensor_msgs::NavSatFix::ConstPtr& msg) { robots[i]->gpsHandler(msg); }
        ));
    }

    // 启动处理线程
    bool use_intraloop_;
    nh.param<bool>("use_intraloop", use_intraloop_, true);
    ...
    for (auto &robot : robots)
    {
        ...
        if (use_intraloop_) {
            std::cout << "Using intraloop processing for robot " << std::endl;
